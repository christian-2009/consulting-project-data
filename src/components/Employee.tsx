...
import { EmployeeInterface, ProjectInterface, ClientInterface } from "./MainContent";
...
export default function Employee(): JSX.Element {
  ...
  useEffect(() => {
    const fetchEmployeeData = async () => {
      ...
      dispatch({type: ACTION.GET_PROJECTS, payload: {projects: projectDataToSet }})
      ...
      dispatch({type: ACTION.GET_CLIENTS, payload: {clients: clientDataToSet }})
    };
    fetchEmployeeData();
  //eslint-disable-next-line
  }, []);
  ...
  function getProjectsEmployeeWorked(employee: string , projects: ProjectInterface[], clients: ClientInterface[]){
  ...
  const projectsEmployeeWorked = getProjectsEmployeeWorked(definedEmployeeId, state.projects, state.clients)
  ...
  return (
    <>
      <div className="body">
        ...
        <img className = 'employee-page--image' src={state.avatar} alt='employee avatar' />
        <p>Projects:</p>
        <ul className = 'employee-page--list'>
        {projectsEmployeeWorked.map((project) => (
          <li className = 'employee-page--list-item' key={project.id}>{project.client}:<br /> {project.contract.startDate} - {project.contract.endDate} <br /></li>
        ))}
        </ul>
        </div>
        </div>
